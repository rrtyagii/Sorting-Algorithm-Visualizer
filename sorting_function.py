...
class sorting_function:
    ...
    def _merge(self, lst, left, midpoint, right, drawData, tickTime):
        drawData(lst, self._colour_codes_mergeSort(len(lst), left, midpoint, right))
        time.sleep(tickTime)
        ...
        for idx in range(left, right + 1):
            if i < len(L) and j < len(R):
                ...
                lst[idx] = R[j]
                j += 1
        drawData(lst, ['#6c9ff0' if x >= left or x <= right else '#8351a1' for x in range(len(lst))])
        time.sleep(tickTime)
    ...
    def _partition(self, arr, start, end, draw_array, timetick):
        ...
        draw_array(arr, self._colour_for_quicksort(len(arr), start, end, i, i))
        time.sleep(timetick)

        for j in range(start, end):
            if arr[j] < pivot:
                draw_array(arr,
                           self._colour_for_quicksort(length=len(arr), left=start, right=end, partition=i, currIdx=j,
                                                      swapped=True))
                time.sleep(timetick)
                arr[i], arr[j] = arr[j], arr[i]
                i += 1

            draw_array(arr, self._colour_for_quicksort(length=len(arr), left=start, right=end, partition=i, currIdx=j))
            time.sleep(timetick)

        draw_array(arr, self._colour_for_quicksort(length=len(arr), left=start, right=end, partition=i, currIdx=end,
                                                   swapped=True))
        time.sleep(timetick)
    ...
    def _colour_codes_mergeSort(self, length, left, middle, right):
        colour_codes = []
    ...
    def _colour_for_quicksort(self, length, left, right, partition, currIdx, swapped=False):
        ...
        for i in range(length):
            if i <= left and i <= right:
                colourArray.append('#f48cfc')
            else:
                colourArray.append('#8351a1')
            if i == right:
                colourArray[i] = "#6c9ff0"
            elif i == partition:
                colourArray[i] = '#8aafe9'
            elif i == currIdx:
                colourArray[i] = '#b5c8eb'

            if swapped:
                if i == partition or i == currIdx:
