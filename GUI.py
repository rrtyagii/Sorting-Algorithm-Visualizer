...
def sorting():
    ...
    if currAlgorithms.get() == 'Merge Sort':
        sortingClass.merge_sort(arr, draw_array, speedScale.get())
...
algMenu = ttk.Combobox(mainFrame, textvariable=currAlgorithms,
...
speedScale = Scale(mainFrame, from_=0.001, to=0.5, length=200, digits=2, resolution=0.001, orient=HORIZONTAL,
...
Button(mainFrame, text='Generate Data', command=random_array_generator, bg='#b5bdc4').grid(row=0, column=2, padx=5,
                                                                                           pady=5)
